...
class Grafo {
        ...
        obtenerDetallesConexiones() {
            let detallesConexiones = "";
        
            for (const [nombrePersona, persona] of this.nodos.entries()) {
                if (persona.amistades && persona.amistades.size > 0) {
                    for (const amigo of persona.amistades.keys()) {
                        detallesConexiones += `${nombrePersona} -> ${amigo.nombre};`;
                    }
                } else {
                    detallesConexiones += `${nombrePersona};`;
                }
            }
        
            return detallesConexiones;
        }
